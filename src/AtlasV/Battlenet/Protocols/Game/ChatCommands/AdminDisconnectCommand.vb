...
Namespace AtlasV.Battlenet.Protocols.Game.ChatCommands
    Class AdminDisconnectCommand
        ...
        Public Overrides Sub Invoke(ByVal varContext As ChatCommandContext)
            ...
            Dim reason = String.Join(" "c, Arguments)
            ...
            r = If(String.IsNullOrEmpty(reason), Resources.DisconnectedByAdmin, Resources.DisconnectedByAdminWithReason.Replace("{reason}", reason))

            For Each kv In env
                r = r.Replace("{" & kv.Key & "}", kv.Value)
            Next

            Call New SID_MESSAGEBOX().Invoke(New MessageContext(target.Client, MessageDirection.ServerToClient, New Dictionary(Of String, Object)() From {
                {"style", CUInt(&H10030)},
                {"caption", Resources.DisconnectedByAdminCaption},
                {"text", r}
            }))
