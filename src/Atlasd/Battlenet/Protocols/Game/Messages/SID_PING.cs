    ...
    class SID_PING : Message
    {
        ...
        public override bool Invoke(MessageContext context)
        {
            ...
            if (token != context.Client.GameState.PingToken)
                throw new ProtocolViolationException(context.Client.ProtocolType, "SID_PING token mismatch with server [Token: 0x" + token.ToString("X8") + "] [Server: 0x" + context.Client.GameState.PingToken.ToString("X8") + "]");
