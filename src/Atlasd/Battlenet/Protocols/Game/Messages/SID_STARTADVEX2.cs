        ...
        public override bool Invoke(MessageContext context)
        {
            switch (context.Direction)
            {
                case MessageDirection.ClientToServer:
                    {
                        Logging.WriteLine(Logging.LogLevel.Debug, Logging.LogType.Client_Game, context.Client.RemoteEndPoint, $"[{Common.DirectionToString(context.Direction)}] SID_STARTADVEX2 ({4 + Buffer.Length} bytes)");
                        ...
                        if (Buffer.Length < 23)
                            throw new GameProtocolViolationException(context.Client, "SID_STARTADVEX2 buffer must be at least 23 bytes");
                        ...
                        if (gameAd == null)
                        {
                            gameAd = new GameAd(context.Client.GameState, gameName, gamePassword, gameStatstring, portNumber, (GameAd.GameTypes)gameType, context.Client.GameState.Version.VersionByte);
