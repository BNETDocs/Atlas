...
namespace Atlasd.Battlenet.Protocols.MCP.Messages
{
    class MCP_CHARLIST2 : Message
    {
        ...
        public MCP_CHARLIST2(byte[] buffer)
        {
            Id = (byte)MessageIds.MCP_CHARLIST2;
            Buffer = buffer;
        }
                ...
                case MessageDirection.ClientToServer:
                    {
                        ...
                        if (Buffer.Length < 4)
                            throw new GameProtocolViolationException(realmState.ClientState, $"{MessageName(Id)} must be at least 4 bytes, got {Buffer.Length}");
                        ...
                        return new MCP_CHARLIST2().Invoke(new MessageContext(realmState, MessageDirection.ServerToClient, new Dictionary<string, object> { { "requested", requested } }));
                    }
                case MessageDirection.ServerToClient:
                    {
                        ...
                        w.Write((UInt16)(characterCount == 0 ? 1 : 12)); // i think this field is documented incorrectly
