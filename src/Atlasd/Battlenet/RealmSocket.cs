...
namespace Atlasd.Battlenet
{
    class RealmSocket : IDisposable
    {
        ...
        public RealmSocket(IPEndPoint localEndPoint)
        {
            IsListening = false;
            Socket = null;
        ...
        {
            var realmState = new RealmState(e.AcceptSocket);

            // Start the read loop on a new stack
            Task.Run(() =>
            {
                realmState.ReceiveAsync();
            });
        ...
        public void Start(int backlog = 100)
        {
            ...
            Logging.WriteLine(Logging.LogLevel.Info, Logging.LogType.Server, $"Starting TCP realm listener on [{LocalEndPoint}]");
